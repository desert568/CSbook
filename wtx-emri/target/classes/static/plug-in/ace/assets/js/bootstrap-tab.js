var addTabs = function (options) {
    ...
    if (!$("#" + id)[0]) {
        ...
        mainHeight = mainHeight-$(".top-nav").outerHeight(true)-$(".navbar-container").outerHeight(true);
        ...
        if (options.content) {
            ...
            content = '<div role="tabpanel" style="margin-bottom: -30px" class="tab-pane" id="' + id + '"><iframe src="' + options.url + '" width="100%" height="' + mainHeight +
        ...
        items: [
            {
                ...
                label:'刷新缓存',icon: _basePath + '/plug-in/diy/icons/refresh.png',action:function(){
            ...
            },
            {
            	...
            	// add-begin--Author:weict  Date:20170614 for：TASK #2108 【ace样式】Ace 风格右键操作太丑-------------------- 
                label:'关闭',icon:_basePath +'plug-in/diy/icons/closeone.png',action:function(){
                // add-end--Author:weict  Date:20170614 for：TASK #2108 【ace样式】Ace 风格右键操作太丑---------------------- 
                //last就是当前选中的元素
            ...
            {
            	// add-begin--Author:weict  Date:20170614 for：TASK #2108 【ace样式】Ace 风格右键操作太丑-------------------- 
            	label:'全部关闭',icon:_basePath +'plug-in/diy/icons/closeall.png',action:function(){
                // add-end--Author:weict  Date:20170614 for：TASK #2108 【ace样式】Ace 风格右键操作太丑----------------------    
                $("#tabs>ul>li").remove();
                $("#tabs>div>div").remove();
                //tabs.tabs("refresh");
            ...
            {
            	// add-begin--Author:weict  Date:20170614 for：TASK #2108 【ace样式】Ace 风格右键操作太丑-------------------- 
                label:'除此之外全部关闭',icon:_basePath +'plug-in/diy/icons/closeother.png',action:function(){
                // add-end--Author:weict  Date:20170614 for：TASK #2108 【ace样式】Ace 风格右键操作太丑----------------------    
                var closeText = last.children("a").text().trim();
                var nowText = $("#tabs").find("li[class='active']").children("a").text().trim();
            ...
            {
            	// add-begin--Author:weict  Date:20170614 for：TASK #2108 【ace样式】Ace 风格右键操作太丑-------------------- 
                label:'当前页右侧全部关闭',icon:_basePath +'plug-in/diy/icons/closeright.png',action:function(){
                // add-end--Author:weict  Date:20170614 for：TASK #2108 【ace样式】Ace 风格右键操作太丑----------------------    
                var closeText = last.children("a").text().trim();
                var nowText = $("#tabs").find("li[class='active']").children("a").text().trim();
                if(closeText==nowText){
            ...
            {
            	// add-begin--Author:weict  Date:20170614 for：TASK #2108 【ace样式】Ace 风格右键操作太丑-------------------- 
            	label:'当前页左侧全部关闭',icon:_basePath +'plug-in/diy/icons/closeleft.png',action:function(){
                // add-end--Author:weict  Date:20170614 for：TASK #2108 【ace样式】Ace 风格右键操作太丑----------------------    
                var closeText = last.children("a").text().trim();
                var nowText = $("#tabs").find("li[class='active']").children("a").text().trim();
                if(closeText==nowText){
