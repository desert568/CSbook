...
(function (layui, window, factory) {
	...
	Common.prototype.addLabel = function (id, div, val) {
		...
		$("select").each(function () {
			var me = $(this);
			/*if(me.attr("xm-select")==id) {
                if (me.attr("xm-select-search") != undefined) {
                    //如果是固定高度
                    div.find('input').css('width', '40px');
                    var num_1 = val.name.length * 15 + 120;
                    if (me.parent().width() < num_1) {
                        var num_2 = me.parent().width() - 110;//带搜索 ×和省略号加上下拉箭头的长度
                        if (val.name.length > num_2 / 15) {
                            name = val.name.substring(0, num_2 / 15) + "..";
                            //如果是固定高度
                            div.find('input').css('width', '30px');
                        }
                    } else {
                        var num_2 = me.parent().width() - 60;//不带搜索 ×和省略号加上下拉箭头的长度
                        if (val.name.length > num_2 / 15) {
                            name = val.name.substring(0, num_2 / 15) + "..";
                        }
                    }
		...
		var tips = 'fsw="' + NAME + '"';
		var _ref = [$('<span ' + tips + ' value="' + val.value + '"><font ' + tips + '>' + name + '</font></span>'), $('<i ' + tips + ' class="xm-iconfont icon-close"></i>')],
		    $label = _ref[0],
		    $close = _ref[1];
		$label.append($close);
		//如果是radio模式
